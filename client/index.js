  ...
  factory(require) {
    ...
    function AdvisorCard({ block, inspect }) {
      ...
      return h('section', { style: styles.card, 'aria-label': '顾问建议', 'data-advisor-card': true },
        ...
        !result ? h('p', { style: styles.text }, '正在按已保存的审批方式处理求助。') : h(React.Fragment, null,
          result.approval && h('p', { style: styles.meta, 'data-advisor-approval': true }, (result.approval.mode === 'always' ? (result.status === 'review_required' ? '全部自动批准 · 已跳过' : '全部自动批准') : result.approval.mode === 'self' ? (result.status === 'review_required' ? '主模型标注 · 已跳过' : '主模型标注 · 自动发送') : result.approval.mode === 'auto' ? (result.status === 'review_required' ? 'AI 审核未批准' : 'AI 已批准') : '人工审批') + (result.approval.reviewer ? ' · ' + result.approval.reviewer : '') + '：' + result.approval.reason),
    ...
    function AdvisorSettingsCard({ scope, describe, remote }) {
      ...
      return h('li', { style: { ...styles.card, listStyle: 'none' }, 'data-advisor-settings': true },
        ...
        h('p', { style: styles.text }, '选择遇到难题时求助的模型。顾问返回建议，主模型负责验证和执行；可配置求助审批方式。'),
        ...
        field('审批方式', h('select', { 'aria-label': '审批方式', style: control, value: value.approvalMode, disabled, onChange: e => edit('approvalMode', e.target.value) }, h('option', { value: 'manual' }, '每次询问'), h('option', { value: 'self' }, '主模型标注（无额外调用）'), h('option', { value: 'auto' }, '独立模型审核（额外调用）'), h('option', { value: 'always' }, '全部自动批准（无额外调用）'))),
        value.approvalMode === 'always' && h(React.Fragment, null,
          h('p', { style: styles.text }, '保存后，所有有效的顾问求助都会直接发送，包括主模型标注需要人工审批、未提供标签或有本地敏感内容提示的请求。不调用审批模型，也不逐次询问。'),
          h('p', { style: styles.meta }, '顾问接收地址：' + (endpoint || '尚未配置') + '\n仍会进行已有脱敏；此设置仅批准文字咨询，文件读取、命令执行等遵循 DSH 权限。地址变化后跳过求助，需在这里重新保存。'),
          value.autoAdvisorEndpoint !== endpoint && h('p', { role: 'status', style: styles.text }, '请保存以启用全部自动批准并确认顾问接收地址。')),
