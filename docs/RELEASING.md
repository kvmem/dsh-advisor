...
npm run check
npm pack
tar -tzf dsh-tool-advisor-0.1.4.tgz
```
...
tar -xzf dsh-tool-advisor-0.1.4.tgz -C "$packed_dir"
...
```sh
dsh plugin --profile web add /absolute/path/to/dsh-tool-advisor-0.1.4.tgz
dsh web
```
