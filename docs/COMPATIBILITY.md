...
Plugin 0.1.11 repeats type checks, builds, 96 regression tests and real Loader tests for all four approval modes across the same 20 combinations. Its approve-all settings UI was checked with DSH 0.1.3-alpha.2 and Chromium. No real cloud models were called.
...
CI uses npm 11.7.0 consistently. Node 24.2.0 bundles npm 11.3.0, which failed the matrix's fresh DSH dependency installation with `ERESOLVE` while resolving the exact-version overrides. To run the matrix with the same npm version without replacing your global npm installation, use `npx --yes --package=npm@11.7.0 npm run check:dsh -- 0.1.5-alpha.1`. This affects the source compatibility test setup; normal plugin installation uses DSH's pnpm installer.
